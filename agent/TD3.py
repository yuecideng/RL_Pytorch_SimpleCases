...
class Actor(nn.Module):
    def __init__(self, s_dim, a_dim):
        ...
        self.linear1 = nn.Linear(s_dim, 400)
        self.linear2 = nn.Linear(400, 300)
        self.linear3 = nn.Linear(300, a_dim)
        self.Relu = nn.ReLU()
...
class Critic(nn.Module):
    def __init__(self, s_dim, a_dim):
        ...
        self.linear1 = nn.Linear(s_dim+a_dim, 400)
        self.linear2 = nn.Linear(400, 300)
        self.linear3 = nn.Linear(300, 1)
        self.linear4 = nn.Linear(s_dim+a_dim, 400)
        self.linear5 = nn.Linear(400, 300)
        self.linear6 = nn.Linear(300, 1)
        self.Relu = nn.ReLU()
        
    def forward(self, x, a):
        x1 = self.linear1(torch.cat([x,a],1))
        x1 = self.Relu(x1)
        x1 = self.linear2(x1)
        x1 = self.Relu(x1)
        x1 = self.linear3(x1)

        x2 = self.linear1(torch.cat([x,a],1))
        x2 = self.Relu(x2)
        x2 = self.linear2(x2)
        x2 = self.Relu(x2)
        x2 = self.linear3(x2)
...
class TD3(object):
    ...
    def Learn(self):
        ...
        #a_ = self.actor_target(s_) + torch.clamp(torch.from_numpy(np.random.normal(0,self.target_noise,self.a_dim)).float(),
        ...
        self.optim_c.step()
        # store Q value in each step for visualizing
