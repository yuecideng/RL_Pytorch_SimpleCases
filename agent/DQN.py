...
class DeepQNetwork:
    def __init__(
        ...
        if self.cuda:
            ...
            self.eval_net = self.model_to_cuda(self.eval_net)
    ...
    def choose_action(self, observation):
        ...
        if self.cuda:
            observation = self.tensor_to_cuda(observation)

        if np.random.uniform() < self.epsilon:
            # forward feed the observation and get q value for every actions
            action_val = self.eval_net(observation)
            if self.cuda:
                action = action_val.detach().cpu().numpy()
            else:
    ...
    def learn(self):
        ...
        if self.prioritized:
            ...
            if self.cuda:
                ISWeights = self.tensor_to_cuda(ISWeights)
        ...
        if self.cuda:
            ...
            action = self.tensor_to_cuda(action)
            reward = self.tensor_to_cuda(reward)
        ...
        if self.prioritized:
            ...
            if self.cuda:
                abs_errors = abs_errors.detach().cpu().numpy()
            else:
    ...
    def model_to_cuda(self, model):
        return model.to(self.cuda_device)
