...
class ArmEnv(object):
    ...
    def render(self):
        if self.viewer is None:
            self.viewer = Viewer(400,400, self.arm_info, self.point_info, self.point_l, self.mouse_in)
        self.viewer.render()
...
class Viewer(pyglet.window.Window):
    ...
    def _update_arm(self):
        ...
        arm1_coord = np.concatenate((self.center_coord, self.arm_info[0, 2:4]),axis=None)  # (x0, y0, x1, y1)
        arm2_coord = np.concatenate((self.arm_info[0, 2:4], self.arm_info[1, 2:4]),axis=None)  # (x1, y1, x2, y2)
